...
def cloud_observation_status() -> dict:
    ...
    return {
        ...
        "chronological": observations.empty or observations["timestamp"].is_monotonic_increasing,
        "latest_timestamp": None if observations.empty else observations["timestamp"].iloc[-1].isoformat(),
    }
...
def ensure_observation_uniqueness(connection) -> None:
    ...
    connection.execute(text(
        f"CREATE UNIQUE INDEX IF NOT EXISTS {TABLE}_city_event_timestamp_key "
...
def insert_observations_conflict_safe(rows: pd.DataFrame) -> int:
    ...
    # Convert nullable float NaN values to actual SQL NULL rather than retaining
    # NaN because of a pandas numeric dtype (important for nullable integer fields).
...
def build_live_observation_row(history: pd.DataFrame, observation: dict) -> pd.DataFrame:
    ...
    serving = latest_serving_features(history, candidate.iloc[0].to_dict())
    if serving.loc[:, ["aqi_lag1", "aqi_lag2", "aqi_lag3", "aqi_rolling_mean"]].isna().any(axis=None):
        raise ValueError("Live observation has insufficient real prior AQI history for lag features.")

    row = candidate.rename(columns={"timestamp": "event_timestamp"}).copy()
    row["event_timestamp"] = row["event_timestamp"].map(to_utc_datetime)
    row["created_timestamp"] = to_utc_datetime(pd.Timestamp.now(tz="UTC"))
...
def ingest_observation(observation: dict) -> pd.DataFrame:
    ...
    candidate = pd.DataFrame([{key: observation[key] for key in ["timestamp", "aqi", *RAW_FEATURES]}])
    candidate["timestamp"] = pd.to_datetime(candidate["timestamp"], utc=True)
    if not history.empty and candidate["timestamp"].iloc[0] in set(history["timestamp"]):
        return candidate.rename(columns={"timestamp": "event_timestamp"})
    row = build_live_observation_row(history, observation)
