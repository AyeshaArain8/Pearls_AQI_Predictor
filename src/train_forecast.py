...
def train() -> tuple[str, dict]:
    ...
    print(f"Training: daily resample produced {len(daily_history)} calendar days (including any gap days as NaN).")

    dataset = make_daily_feature_rows(daily_history, include_targets=True)
    if len(dataset) < 15:
        raise ValueError(
            f"Only {len(dataset)} usable daily Lahore rows after resampling; "
            "need more consecutive days of history before a real 3-day model can be trained."
        )

    split = int(len(dataset) * .8); train_rows, test_rows = dataset.iloc[:split], dataset.iloc[split:]
    print(f"Training: {len(dataset)} usable daily rows; {len(train_rows)} train rows; {len(test_rows)} test rows.")
    ...
    metadata = {
        ...
        "metrics": metrics,
        "data_source": "Feast cloud PostgreSQL historical retrieval, resampled to one row per calendar day",
        "forecast_granularity": "daily (day1=+24h, day2=+48h, day3=+72h)",
