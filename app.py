...
def health_advice(category):
    ...
    if "Good" in category:
        ...
        return "😷 Sensitive people should reduce prolonged outdoor exposure."

    elif "Unhealthy" in category:
        return "⚠️ Limit outdoor activities and wear a mask if needed."

    elif "Very" in category:
        return "🚨 Avoid outdoor activities."

    else:
        return "☠️ Stay indoors and avoid outdoor exposure."
...
st.title("🌍 Air Quality Index - 3 Day Forecast")
...
if predict:
    ...
    result = forecast_aqi(
        ...
        city_data["pm10"],
        city_data["pm2_5"],
        city_data["carbon_monoxide"],
        city_data["nitrogen_dioxide"],
        city_data["sulphur_dioxide"],
        city_data["ozone"],
        ...
        current_aqi=city_data["openweather_aqi"]
    ...
    chart = alt.Chart(chart).mark_line(
        point=True
    ).encode(

        x=alt.X("Day", title="Forecast Day"),
    ...
    for p in priority:
        if any(p in c for c in categories):
            selected = p
            break
